...
function EditConfigureWatch(props){
    ...
    const getTheCampaignList = async () => {
        ...
        console.log(res)
        let data_response = res.data.body
        setManagerList(data_response)
    }
        catch(err){
    ...
    const handleChange = (e, key) =>{
        if(key == "campaign_id"){
            setTempCampaignId(JSON.parse(e.target.value).campaignid)
        }
        
        const newObject = {...configureDetails}
        if(key == "campaign_id"){
            newObject[key] = JSON.parse(e.target.value).campaignid
        }else{
    ...
    const handleParticipantChange = async (e, campaign_id) =>{
        try{
            const response = await axios.post("https://aa2397tzu2-vpce-00569c5e62069a9a0.execute-api.us-east-1.amazonaws.com/roamm/new_get_watch_config",{
            //manager_id : localStorage.getItem("managerId"),
            campaign_id : currentCampaign["campaignid"], //localStorage.getItem("campaignId")
        ...
        console.log(response.data)
            ...
            const newObject = {...configureDetails}
            newObject["starttime"] = data[3]
            newObject["endtime"] = data[4]
            newObject["watchid"] = data[0]
            newObject["participant_id"] = e.target.value
            newObject["campaign_id"] = campaign_id
            setConfigureDetails(newObject)
        }
        catch(error){
            setAlertShow([true,"danger"])
    ...
    const handleConfigureWatch = async (configDetails) => {
        try{
            const response = await axios.post("https://aa2397tzu2-vpce-00569c5e62069a9a0.execute-api.us-east-1.amazonaws.com/roamm/new_update_watch_config",configDetails)
            if(response.data.statusCode != 200){
                setAlertShow([true,"danger"])
                setErrorMsg(JSON.parse(response.data.body).message)
            }
    ...
    const handleCampaignChange = async () =>{
        ...
        try{
            ...
            manager_id : currentCampaign["managerid"],
            campaign_id : currentCampaign["campaignid"]
        })
        if(response.data.statusCode != 200){
            setAlertShow([true,"danger"])
            setErrorMsg(JSON.parse(response.data.body).message)
            ...
            <div style={{height:"inherit"}}>
                <Container style={{height:"100%"}}>
                    <Row style={{height:"100%"}}>
                        ...
                        <SideNavbar list={["Coordinator Dashboard","Edit Watch Configuration"]} active={1} links={["/coordinatorDashboard","/editConfigureWatchCoordinator"]}/>
                        </Col>
                        <Col style={{height: "75%"}}>
                        {alertShow[0]?(alertShow[1] == "danger"? <AlertBox type="danger" message={errorMsg}/>:<></>):<></>}
                            <h2>Edit Configure Watch</h2>
                            <hr/>
                            <Container className="p-4" style={{background:"rgba(255,255,255,0.3)",height:"50%",overflowY:"auto"}}>
                                
                            
                                        <Col className='mt-3'>
                                            <Form.Group>
                                                <Form.Label>Select Campaign</Form.Label>
                                            <Form.Select style={{background:"rgba(0,0,0,0.05)"}} onChange={(e)=>{handleChange(e, "campaign_id"); setCurrentCampaign(JSON.parse(e.target.value))}}>
                                                <option value="">select option</option>
                                                {managerList.map((data,index)=>
                                                    (<option key={index} value={JSON.stringify(data)}>{data.campaignid}</option>)
                                                )}
                                            </Form.Select>
                                            </Form.Group>
                                        </Col>
                                        ...
                                        <Col className='mt-3'>
                                            ...
                                            <Form.Select style={{background:"rgba(0,0,0,0.05)"}}  onChange={(e)=>{handleChange(e, "participant_id"); handleParticipantChange(e, configureDetails.campaign_id)}}>
                                    ...
                                    <hr></hr>
                                   {showForm && <Container className="p-4" style={{background:"rgba(255,255,255,0.3)",height:"inherit",overflowY:"auto"}}>
                                    <Form onSubmit={(e)=>{e.preventDefault(); handleSubmit()}}>
                                        <Col className='mt-3'>
                                        <Form.Group>
                                            <Form.Label>Enter Watch ID</Form.Label>
                                            <Form.Control value={configureDetails.watchid} type="text" style={{background:"rgba(0,0,0,0.05)"}}  onChange={(e)=>{handleChange(e, "watchid")}}/>
                                        </Form.Group>
                                        </Col>
                                        ...
                                        <Form.Group>
                                            ...
                                            <Form.Control value={configureDetails.starttime} type="datetime-local" style={{background:"rgba(0,0,0,0.05)"}}  onChange={(e)=>{handleChange(e, "starttime")}}/>
                                        </Form.Group>
                                        </Col>
                                        ...
                                        <Form.Group>
                                            ...
                                            <Form.Control value={configureDetails.endtime} type="datetime-local" style={{background:"rgba(0,0,0,0.05)"}}  onChange={(e)=>{handleChange(e, "endtime")}}/>
                                        </Form.Group>
                                        </Col>
...
export default connect(mapStateToProps,mapDispatchToProps)(EditConfigureWatch);
